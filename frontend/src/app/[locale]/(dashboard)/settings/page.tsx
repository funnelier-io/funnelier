...
export default function SettingsPage() {
  ...
  const INFRA_SOURCES = [
    { name: "PostgreSQL", description: t("infraPostgresql"), icon: "🐘", sourceType: "postgresql" },
    { name: "Redis", description: t("infraRedis"), icon: "🔴", sourceType: "redis" },
    { name: "MongoDB", description: t("infraMongodb"), icon: "🍃", sourceType: "mongodb" },
    { name: "Kavenegar", description: t("infraSmsProvider"), icon: "💬", sourceType: "sms_provider" },
    { name: "Asterisk (VoIP)", description: t("infraVoip"), icon: "📞", sourceType: "voip" },
    { name: t("infraFileName"), description: t("infraFile"), icon: "📊", sourceType: "file" },
  ];
  ...
  for (const cfg of apiConfigs) {
    if (!INFRA_SOURCES.some((i) => i.sourceType === cfg.source_type)) {
      mergedSources.push({
        ...
        status: cfg.is_active ? "connected" as const : "disconnected" as const,
  ...
  const uploadEndpoints: Record<string, { path: string; label: string; accept: string }> = {
    leads: { path: "/import/leads/upload", label: t("uploadLeads"), accept: ".xlsx,.xls" },
    calls: { path: "/import/calls/upload", label: t("uploadCalls"), accept: ".csv" },
    sms: { path: "/import/sms/upload", label: t("uploadSms"), accept: ".csv" },
    voip: { path: "/import/voip/upload", label: t("uploadVoip"), accept: ".json,.txt" },
  };
  ...
  async function handleUpload() {
    ...
    try {
      ...
      if (res.ok) {
        setUploadResult({
          ok: true,
          message: t("uploadSuccess", { name: uploadFile_.name }),
        });
        setUploadFile(null);
        ...
        setUploadResult({
          ok: false,
          message: (res.data as Record<string, string>)?.detail || t("uploadError"),
        });
      }
    } catch {
      setUploadResult({ ok: false, message: t("serverError") });
    }
    setUploading(false);
  }
  ...
  const importTypeLabels: Record<string, string> = {
    leads: t("typeLeads"),
    call_logs: t("typeCallLogs"),
    sms_logs: t("typeSmsLogs"),
    voip: "VoIP",
    invoices: t("typeInvoices"),
  };
    ...
    },
    {
      ...
      render: (h: ImportHistory) => (
        <span className="text-xs bg-gray-100 px-2 py-0.5 rounded">
          {importTypeLabels[h.import_type] || h.import_type}
        </span>
      ),
    },
    {
      ...
      render: (h: ImportHistory) => {
        ...
        return (
          <span className={`inline-block px-2 py-0.5 rounded-full text-xs ${colors[h.status] || "bg-gray-50"}`}>
            {importStatusLabels[h.status] || h.status}
          </span>
        );
      },
    },
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          {dataSources.isLoading && (
            <span className="text-xs text-gray-400">{t("loadingText")}</span>
          )}
        </div>
      ...
      <div className="bg-white rounded-lg shadow p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          >
            {scanLoading ? t("scanning") : t("scanFolder")}
          </button>
        </div>

        {scanError && (
          <div className="text-red-500 text-sm bg-red-50 rounded-lg p-3 mb-3">{scanError}</div>
        )}

        {scanResult && (
          <div>
            <p className="text-xs text-gray-400 mb-3">
              {t("folder")} <span dir="ltr" className="font-mono">{scanResult.folder}</span>
              {" — "}
              {t("filesFound", { count: scanResult.files.length })}
            </p>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {scanResult.files.map((f) => (
                <div key={f.name} className="flex items-center justify-between p-2.5 bg-gray-50 rounded-md text-xs">
                  <span className="truncate ml-2">{f.category}</span>
                  <span className="text-gray-400 shrink-0" dir="ltr">{f.size_kb.toFixed(1)} KB</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-white rounded-lg shadow p-5">
        ...
        <div
          ...
          onDrop={(e) => { e.preventDefault(); e.currentTarget.classList.remove("border-blue-400", "bg-blue-50"); const file = e.dataTransfer.files[0]; if (file) setUploadFile(file); }}
        >
          <input
            ref={fileInputRef}
            type="file"
          ...
          {uploadFile_ ? (
            ...
            <div className="space-y-2">
              ...
              <div className="text-sm text-gray-500">{t("dragOrClick")}</div>
              <div className="text-xs text-gray-400">{t("allowedFormats", { formats: uploadEndpoints[uploadType].accept })}</div>
            </div>
          )}
        </div>
        ...
        <div className="mt-4 flex items-center justify-between">
          ...
          {uploadResult && (
            <div className={`text-sm px-3 py-1 rounded-lg ${uploadResult.ok ? "bg-green-50 text-green-700" : "bg-red-50 text-red-700"}`}>
      ...
      <div className="bg-white rounded-lg shadow p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-gray-700">{t("importHistory")}</h2>
...
function DataSourceCard({
  name, description, status, icon, lastSyncAt, recordsSynced, t,
}: {
  name: string; description: string; status: "connected" | "disconnected" | "configured";
  ...
  const statusConfig = {
    ...
    configured: { label: t("configured"), color: "bg-yellow-500", textColor: "text-yellow-700" },
  };
