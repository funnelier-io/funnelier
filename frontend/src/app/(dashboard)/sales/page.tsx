...
export default function SalesPage() {
    ...
    },
    {
      ...
      render: (inv: Invoice) => (
        <span className="text-xs text-gray-500">
          {fmtNum(inv.line_items?.length ?? 0)} قلم
        </span>
      ),
    },
    {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold">فروش</h1>
        ...
        <StatCard
          ...
          subtitle={
            stats.data?.conversion_rate
              ? `نرخ تبدیل: ${(stats.data.conversion_rate * 100).toFixed(1)}%`
              : undefined
          }
        />
      </div>
            ...
            <div className="bg-green-50 rounded-lg px-4 py-3 flex items-center gap-4">
              ...
              <div>
                <div className="text-sm text-green-800 font-semibold">
                  مجموع پرداخت‌ها: {fmtCurrency(payments.data.total_amount)}
                </div>
                <div className="text-xs text-green-600">
                  {fmtNum(payments.data.total_count)} تراکنش ثبت شده
                </div>
              </div>
            </div>
          )}
